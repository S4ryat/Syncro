...
@app.post(
    "/music", response_model=schemas.Music, status_code=HTTP_201_CREATED, tags=["music"]
)
...
def list_recommendations(db: Session = Depends(get_db), skip: int = 0, limit: int = 100) -> Any:
    recommendations = actions.recommendation.get_all(db=db, skip=skip, limit=limit)
    return recommendations


@app.post(
    "/recommendations", response_model=schemas.Recommendation, status_code=HTTP_201_CREATED, tags=["recommendations"]
)
